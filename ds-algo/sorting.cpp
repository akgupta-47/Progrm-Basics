...
sorted list has n^2 time
and unsorted list has nlogn time
*/
#include <iostream>
...
void quick_sort(int Qr*,int l,int h){
    int pivot=Qr[l];
    int st,ed;
    while(!checkSorted(Qr,h)){
        do{i++;} while ( Qr[i]<=pivot );
        do{j--;} while ( Qr[j]>pivot );
